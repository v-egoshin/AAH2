...
type MarkKind = "SOURCE" | "SINK" | "GUARD" | "TRANSFORM" | "NOTE";
...
function candidateKind(candidate: ReviewEntity): MarkKind | null {
...
function matchingCandidateForKind(payload: ReviewContextResponse | null, kind: MarkKind) {
  return (payload?.candidates ?? []).find((candidate) => candidate.candidate_type === "MARK" && candidateKind(candidate) === kind);
}

function currentMarkForKind(payload: ReviewContextResponse | null, kind: MarkKind) {
  return (payload?.marks ?? []).find((mark) => mark.kind === kind);
}

function currentMark(payload: ReviewContextResponse | null) {
...
function oppositeKind(kind: MarkKind): MarkKind | null {
  if (kind === "SOURCE") {
    return "SINK";
  }
  if (kind === "SINK") {
...
function getRecentOppositeMark(kind: MarkKind) {
  const opposite = oppositeKind(kind);
  return opposite ? getRecentMarks(opposite)[0] : null;
}

function getCurrentMarkForTarget(provider: SelectionActionCodeLensProvider, target?: SelectionTarget) {
  if (!target) {
...
async function getMarkContext(editor: vscode.TextEditor, target: SelectionTarget) {
  const padding = Math.max(0, vscode.workspace.getConfiguration("appsecWorkbench").get<number>("markContextPadding", 10));
...
async function createMark(kind: MarkKind, provider: SelectionActionCodeLensProvider, forcePrompt = false, explicitEditor?: vscode.TextEditor, explicitTarget?: SelectionTarget) {
...
async function createCaseFromContext(provider: SelectionActionCodeLensProvider, editor?: vscode.TextEditor, explicitTarget?: SelectionTarget) {
  ...
  const payload = getScopedPayload(provider, target);
  const mark = currentMark(payload);
  const recentOpposite = mark?.kind ? getRecentOppositeMark(mark.kind as MarkKind) : null;
...
async function showSelectionActions(provider: SelectionActionCodeLensProvider, editor?: vscode.TextEditor, explicitTarget?: SelectionTarget) {
  ...
  items.push({
    ...
    run: async () => {
      ...
      await createMark(customKind.markKind as MarkKind, provider, true, targetEditor, target);
    },
  });
...
export function registerMarkCommands(
  ...
  const markCommand = (command: string, kind: MarkKind) => {
    context.subscriptions.push(vscode.commands.registerCommand(command, async (target?: SelectionTarget) => {
  ...
  context.subscriptions.push(vscode.commands.registerCommand("appsecWorkbench.markAny", async (target?: SelectionTarget) => {
  ...
  context.subscriptions.push(vscode.commands.registerCommand("appsecWorkbench.createCheckFromSelection", async (target?: SelectionTarget) => createCheckFromSelection(codeLensProvider, undefined, target)));
  context.subscriptions.push(vscode.commands.registerCommand("appsecWorkbench.attachEvidenceFromSelection", async (target?: SelectionTarget) => attachEvidenceFromSelection(codeLensProvider, undefined, target)));
  context.subscriptions.push(vscode.commands.registerCommand("appsecWorkbench.createCaseFromContext", async (target?: SelectionTarget) => createCaseFromContext(codeLensProvider, undefined, target)));
  context.subscriptions.push(vscode.commands.registerCommand("appsecWorkbench.removeCurrentMark", async (target?: SelectionTarget) => removeCurrentMark(codeLensProvider, target)));
  ...
  context.subscriptions.push(vscode.commands.registerCommand("appsecWorkbench.showRecentMarkActions", async (target?: SelectionTarget) => showRecentMarkActions(codeLensProvider, target)));
  ...
  context.subscriptions.push(vscode.commands.registerCommand("appsecWorkbench.showSelectionActions", async (target?: SelectionTarget) => showSelectionActions(codeLensProvider, undefined, target)));
